...
def getEdgeMap(file, zloc, nodule_list):
  ...
  for onenoduleid in nodule_list:
    ...
    rangelist, Img, pos = getEdgeMap_id(root, onenoduleid, zloc)
    if rangelist != None:
      break
  ...
  for j in range(len(rangelist)):
    lnglist = []
    latlist = []
    for i in range(len(rangelist[j])-1):
      lnglist.append(rangelist[j][i][0])
      latlist.append(rangelist[j][i][1])
    ...
    for a in range(minlng, maxlng):
      for b in range(minlat, maxlat):
        point = [a, b]
        if isPointinPolygon(point, rangelist[j], maxlng, minlng, maxlat, minlat):
          Img[j][b][a] = 1

  return Img, signtemp, pos
              ...
              if  unblindedReadNodule.tag=='{http://www.nih.gov}roi':
                ...
                for roi in unblindedReadNodule:
                  ...
                  if roi.tag == '{http://www.nih.gov}imageZposition':
                    ...
                    rangelist.append(coo)
                    Images.append(Img)
                #flag = False
  return rangelist, Images, pos
